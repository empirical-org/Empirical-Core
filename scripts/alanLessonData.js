  ...
  _.each(hashToCollection(snapshot.val()), function (value) {
    ...
    ref.orderByChild("lessonID").startAt(value.key).endAt(value.key).on("value", function(snapshot) {
      ...
      _.each(hashToCollection(snapshot.val()), function (studentValue) {
        ...
        console.log("Prompt\tAttempt 1\tFeedback 1\tGrade 1\tEdits 1\tAttempt 2\tFeedback 2\tGrade 2\tEdits 2\tAttempt 3\tFeedback 3\tGrade 3\tEdits 3")
        _.each(studentValue.questions, function (question) {
          ...
          _.each(question.attempts, function (attempt, index) {
            ...
            string += '\t' + attempt.submitted + '\t' + ((attempt.response && (attempt.response.feedback != undefined)) ? attempt.response.feedback : '') + '\t' + (attempt.response ? attempt.response.optimal : '') + "\t" + diffs
            // console.log(attempt)
          })
