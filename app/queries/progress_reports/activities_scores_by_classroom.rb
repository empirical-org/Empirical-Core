class ProgressReports::ActivitiesScoresByClassroom
  def self.results(classroom_ids)
    ActiveRecord::Base.connection.execute(
      "SELECT classrooms.name AS classroom_name, students.id AS student_id, students.name, AVG(activity_sessions.percentage) AS average_score, COUNT(activity_sessions.id) AS activity_count FROM classroom_activities
      ...
      GROUP BY classrooms.name, students.id, students.name").to_a
  end
end
