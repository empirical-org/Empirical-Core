class ProgressReports::ActivitiesScoresByClassroom
  def self.results(classroom_ids)
    ActiveRecord::Base.connection.execute(
      "SELECT classrooms.name AS classroom_name, students.id AS student_id, students.last_active, students.name, AVG(activity_sessions.percentage) AS average_score, COUNT(activity_sessions.id) AS activity_count, classrooms.id AS classroom_id FROM classroom_activities
