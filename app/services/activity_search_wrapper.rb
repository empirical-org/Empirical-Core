class ActivitySearchWrapper
  ...
  def search
    ...
    @topics = @activities.includes(topic: [:topic_category, :section]).map(&:topic).uniq.compact
    # @topic_categories = @topics.includes(:topic_category).map(&:topic_category).uniq.compact
