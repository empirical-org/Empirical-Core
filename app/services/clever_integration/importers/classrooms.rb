module CleverIntegration::Importers::Classrooms
  ...
  def self.run(teacher, district_token)
    clever_teacher = self.fetch_clever_teacher(teacher.clever_id, district_token)
    sections_response = clever_teacher.sections
    parsed_sections_response = self.parse_sections_response(sections)
  ...
  def self.fetch_clever_teacher(teacher_clever_id, district_token)
    Clever::Teacher.retrieve(teacher_clever_id, district_token)
  end
