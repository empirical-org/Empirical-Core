class AssignRecommendationsWorker
  ...
  def perform(unit_template_id, classroom_id, student_id_array, last, lesson, assign_on_join=false)
    ...
    units = Unit.unscoped.where(unit_template_id: unit_template_id, user_id: teacher.id) || Unit.unscoped.where(name: UnitTemplate.find(unit_template_id).name, user_id: teacher.id)
    if units.length > 1
    ...
    ClassroomActivity.unscoped.where(unit_id: unit.id).each { |ca| ca.update(visible: true) } if unit
