  ...
  def units_i_own_or_coteach(own_or_coteach, report, lessons)
    ...
    if own_or_coteach_classrooms_array.any?
      ...
      ActiveRecord::Base.connection.execute("SELECT units.name AS unit_name,
         ...
         (SELECT COUNT(DISTINCT user_id) FROM activity_sessions WHERE state = 'finished' AND classroom_activity_id = ca.id) AS completed_count,
         (SELECT COUNT(DISTINCT user_id) FROM activity_sessions WHERE state = 'started' AND classroom_activity_id = ca.id) AS started_count,
