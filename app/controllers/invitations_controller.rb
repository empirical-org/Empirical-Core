class InvitationsController < ApplicationController
  ...
  def create_coteacher_invitation
    begin
      ...
      @pending_invite = Invitation.find_or_create_by(inviter_id: current_user.id, invitee_email: invitee_email, invitation_type: Invitation::TYPES[:coteacher], status: Invitation::STATUSES[:pending])
  ...
  def destroy_pending_invitations_to_specific_invitee
    begin
      Invitation.find_by(invitation_type: params[:invitation_type], inviter_id: current_user.id, invitee_email: params[:invitee_email], status: Invitation::STATUSES[:pending]).destroy
      return render json: {}
    rescue => e
      return render json: { error: e.message }, status: 422
    end
  ...
  def destroy_pending_invitations_from_specific_inviter
    begin
      Invitation.find_by(invitation_type: params[:invitation_type], inviter_id: params[:inviter_id], invitee_email: current_user.email, status: Invitation::STATUSES[:pending]).destroy
      return render json: {}
    rescue => e
      return render json: { error: e.message }, status: 422
    end
