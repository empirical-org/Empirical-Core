...
describe UserLoginWorker, type: :worker do
  ...
  it 'sends a segment.io event when a student logs in' do
    worker.perform(student.id, "127.0.0.1")

    expect(analytics.backend.track_calls.size).to eq(1)
    expect(analytics.backend.track_calls[0][:event]).to eq(SegmentIo::Events::STUDENT_SIGNIN)
    expect(analytics.backend.track_calls[0][:user_id]).to eq(student.id)
  end
end