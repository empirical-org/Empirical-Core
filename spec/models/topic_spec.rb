...
describe Topic, type: :model do
	...
	let!(:topic){FactoryBot.create(:topic, name: "a")}

  it_behaves_like 'uid'

	context "when the default order is by name" do

		let!(:topic1){FactoryBot.create(:topic, name: "c")}
		let!(:topic2){FactoryBot.create(:topic, name: "b")}
	...
	context "when it's updated/created" do
		...
		context "when it runs validations" do
			...
			it "must have a unique name" do
				t=Topic.first
				n=FactoryBot.build(:topic, name: t.name)
				n.valid?
