...
CREATE SEQUENCE public.sales_accounts_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
ALTER SEQUENCE public.sales_accounts_id_seq OWNED BY public.sales_accounts.id;
...
ALTER TABLE ONLY public.sales_accounts ALTER COLUMN id SET DEFAULT nextval('public.sales_accounts_id_seq'::regclass);
...
CREATE INDEX index_sales_accounts_on_school_id ON public.sales_accounts USING btree (school_id);
...
ALTER TABLE ONLY public.sales_accounts
    ADD CONSTRAINT fk_rails_3edb0acfb0 FOREIGN KEY (school_id) REFERENCES public.schools(id);
