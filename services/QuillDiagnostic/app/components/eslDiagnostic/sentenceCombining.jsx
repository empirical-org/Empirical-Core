...
class ELLSentenceCombining extends React.Component {
  ...
  render = () => {
    ...
    if (responses && Object.keys(responses).length) {
      if (question.attempts.length > 0) {
        button = <button className="quill-button focus-on-light large primary contained" onClick={this.handleNextQuestionClick} tabIndex="0" type="button">{this.getSubmitButtonText()}</button>;
      } else {
        button = <button className="quill-button focus-on-light large primary contained" onClick={this.handleSubmitResponse} tabIndex="0" type="button">{this.getSubmitButtonText()}</button>;
      }
    } else {
      button = <button className="quill-button focus-on-light large primary contained disabled" tabIndex="0" type="button">{this.getSubmitButtonText()}</button>;
    }
    if (question) {
