...
export const completeActivitySession = (sessionID, activityId, percentage, conceptResults, data, callback) => {
  return (dispatch: Function) => {
    const activitySessionUrl = `${import.meta.env.VITE_DEFAULT_URL}/api/v1/activity_sessions/${sessionID}`
...
export const saveActivitySurveyResponse = ({ sessionID, activitySurveyResponse, callback, }) => {
  requestPost(
    `${import.meta.env.VITE_DEFAULT_URL}/api/v1/activity_survey_responses`,
    {
...
export const reportAProblem = ({ sessionID, entry, report, callback, isOptimal }) => {
  const reportAProblemUrl = `${import.meta.env.VITE_DEFAULT_URL}/api/v1/student_problem_reports`
...
export const getFeedback = (args: GetFeedbackArguments) => {
  ...
  return (dispatch: Function) => {
    const feedbackURL = `${import.meta.env.VITE_DEFAULT_URL}/api/v1/evidence/feedback/`
