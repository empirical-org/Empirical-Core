...
export const getActivity = (sessionID: string, activityUID: string) => {
  return async (dispatch: Function) => {
    ...
    const activityUrl = `${process.env.VITE_DEFAULT_URL}/api/v1/evidence/activities/${activityUID}`
...
export const getTopicOptimalInfo = (activityID: number) => {
  return async (dispatch: Function) => {
    const topicOptimalInfoUrl = `${process.env.VITE_DEFAULT_URL}/api/v1/evidence/activities/${activityID}/topic_optimal_info`
