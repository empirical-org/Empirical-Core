...
export default class StudentAccount extends Component {
  ...
  updateUser = (data, url, snackbarCopy, errors) => {
    if(!_.isEmpty(errors)) {
      ...
      requestPut(`${process.env.VITE_DEFAULT_URL}${url}`, dataObject, (body) => {
        const {
          name,
          username,
          email,
    ...
    return(
      <div className="user-account-section third-party-integration-explanation">
        <div className="third-party-integration-explanation header">
          ...
          <img alt="Lightbulb with rays shining" src={`${process.env.VITE_PROCESS_ENV_CDN_URL}/images/illustrations/bulb.svg`} />
        </div>
  ...
  renderLinkedAccount = () => {
    ...
    if(cleverId || googleId ) {
      ...
      const path = `${process.env.VITE_PROCESS_ENV_CDN_URL}/images/shared/${label}_icon.svg`
      return(
