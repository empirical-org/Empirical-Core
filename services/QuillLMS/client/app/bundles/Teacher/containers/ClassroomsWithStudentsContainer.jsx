...
import { Snackbar, defaultSnackbarTimeout, LoadingSpinner, } from '../../Shared/index';
...
const UpdateAssignedStudents = ({ match, user, }) => {
  ...
  function getClassroomsAndStudentsData() {
    requestGet(`/teachers/units/${match.params.unitId}/classrooms_with_students_and_classroom_units`, (body) => {
      const newAssignmentData = body.classrooms.map(c => ({
        ...
        assign_on_join: c.classroom_unit.assign_on_join
      }))
  ...
  const classrooms = originalClassrooms.map(classroom => {
    ...
    const students = classroom.student.map(s => (
      <Student
  ...
  return (
    <section className="update-assigned-students-container">
...
export default class ClassroomsWithStudentsContainer extends React.Component {
  ...
  getClassroomsAndStudentsData = () => {
    ...
    requestGet(url, (body) => {
      that.setState({ loading: false, classrooms: body.classrooms, originalClassrooms: body.classrooms, unitName: unitName(body), });
