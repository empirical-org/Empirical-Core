...
import { ALPHA_FLAG, ARCHIVED_FLAG, BETA_FLAG, DropdownInput, GAMMA_FLAG, Input, NOT_APPLICABLE, PRIVATE_FLAG, PRODUCTION_FLAG, Snackbar, TextEditor, Tooltip, defaultSnackbarTimeout } from '../../../Shared/index';
...
export const UnitTemplate = ({ unitTemplate }) => {
  ...
  function handleFileDrop(acceptedFiles) {
    acceptedFiles.forEach(file => {
      ...
      fetch(`${process.env.VITE_DEFAULT_URL}/cms/images`, {
        method: 'POST',
        mode: 'cors',
        credentials: 'include',
        headers: {
  ...
  function renderActivityPackPreviewLink() {
    if(!id) { return };
    const url = `${process.env.VITE_DEFAULT_URL}/assign/featured-activity-packs/${id}`;
    return(
