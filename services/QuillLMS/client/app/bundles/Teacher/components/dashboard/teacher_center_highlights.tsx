...
const Highlight = ({ blogPost, }) => {
  ...
  return (<a className="highlight focus-on-light" href={link}>
...
const TeacherCenterHighlights = ({ featuredBlogPosts, }) => {
  return (<section className="teacher-center-highlights">
    <h2>Teacher center highlights</h2>
    {featuredBlogPosts.map(bp => <Highlight blogPost={bp} key={bp.id} />)}
    <a className="see-all-resources focus-on-light" href={`/${TEACHER_CENTER_SLUG}`}>See all resources</a>
  </section>)
}
