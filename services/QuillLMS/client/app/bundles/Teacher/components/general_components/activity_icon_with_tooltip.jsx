...
export default class ActivityIconWithTooltip extends React.Component {
  ...
  getConceptResultInfo() {
    const that = this;
    requestGet(
      `${process.env.VITE_DEFAULT_URL}/grades/tooltip/classroom_unit_id/${this.props.data.cuId}/user_id/${this.props.data.userId}/activity_id/${this.activityId()}/completed/${!!this.props.data.completed_attempts}`,
      (body) => {
  ...
  missedIndicator() {
    ...
    if (marked_complete && completed_attempts === 0) {
      return <img alt="" className="missed-indicator" src={`${process.env.VITE_PROCESS_ENV_CDN_URL}/images/scorebook/missed-lessons-cross.svg`} />
    }
  }
