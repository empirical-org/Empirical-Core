...
export default class UnarchiveUnits extends React.Component {
  ...
  getArchivedUnits = () => {
    ...
    requestGet(
      `${process.env.VITE_DEFAULT_URL}/teacher_fix/archived_units?teacher_identifier=${that.state.teacherIdentifier}`,
      (body) => {
  ...
  unarchiveUnits = () => {
    const that = this
    requestPost(
      `${process.env.VITE_DEFAULT_URL}/teacher_fix/unarchive_units`,
