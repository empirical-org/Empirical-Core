...
export default class ActivityPacks extends React.Component {
  ...
  getUnits = () => {
    requestGet(
      `${process.env.VITE_DEFAULT_URL}/teachers/units?report=true`,
      (body) => {
  ...
  populateCompletionAndAverageScore = (data) => {
    ...
    const requests = data.map((u) => {
      return new Promise(resolve => {
        requestGet(
          `${process.env.VITE_DEFAULT_URL}/teachers/units/score_info_for_activity/${u.activity_id}?classroom_unit_id=${u.classroom_unit_id}`,
          (body) => {
