...
export default class ClassroomActivity extends React.Component {
  ...
  anonymousPath = () => {
    return `${process.env.VITE_DEFAULT_URL}/activity_sessions/anonymous?activity_id=${this.activityId()}`;
  }
  ...
  handleLaunchLessonWithNoStudents = () => {
    if (window.confirm("You have no students in this class. Quill Lessons is a collaborative tool for teachers and students to work together. If you'd like to launch this lesson anyway, click OK below. Otherwise, click Cancel.")) {
      window.location.href = `${process.env.VITE_DEFAULT_URL}/teachers/classroom_units/${this.classroomUnitId()}/launch_lesson/${this.activityId()}`;
    }
  }
  ...
  lessonCompletedOrLaunch = () => {
    ...
    if (data.started) {
      return <a className="resume-lesson" href={`${process.env.VITE_DEFAULT_URL}/teachers/classroom_units/${this.classroomUnitId()}/launch_lesson/${this.activityId()}`}>Resume Lesson</a>;
    }
    return <a href={`${process.env.VITE_DEFAULT_URL}/teachers/classroom_units/${this.classroomUnitId()}/launch_lesson/${this.activityId()}`} id="launch-lesson">Launch Lesson</a>;
  }
