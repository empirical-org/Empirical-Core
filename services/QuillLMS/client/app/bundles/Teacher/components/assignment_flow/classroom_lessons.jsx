...
export default class ClassroomLessons extends React.Component {
  ...
  getAllLessons = () => {
    requestGet(`${process.env.VITE_DEFAULT_URL}/teachers/lesson_units`, (body) => {
      this.setState({ allLessons: body, }, () => this.getLessonsForCurrentClass());
    });
  }

  getClassrooms = (classroomId) => {
    requestGet(`${process.env.VITE_DEFAULT_URL}/teachers/classrooms_i_teach_with_lessons`, (body) => {
  ...
  renderEmptyState() {
    ...
    return (
      <div className="empty-lessons manage-units gray-background-accommodate-footer">
        ...
        <img alt="cartoon of a teacher gesturing at a projector screen showing Quill Lessons content" src={`${process.env.VITE_PROCESS_ENV_CDN_URL}/images/illustrations/empty_state_illustration_lessons.svg`} />
      </div>
    );
  }
  ...
  renderHeader() {
    ...
    const paragraphWithLinks = <p>Before you launch a lessons activity with your students, we recommend you check out <a href={`${process.env.VITE_DEFAULT_URL}/tutorials/lessons/1`} target="_blank">this tutorial</a> on how to lead a lesson. We have also put together a <a href="https://support.quill.org/using-quill-tools/quill-lessons/getting-started-how-to-set-up-your-first-quill-lesson" target="_blank">comprehensive guide</a> that will explain how to set up lessons in your classroom.</p>
