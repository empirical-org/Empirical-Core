...
import { DropdownInput, Snackbar, Spinner, defaultSnackbarTimeout } from '../../../../../Shared/index';
import { DropdownObjectInterface } from '../../../../../Staff/interfaces/evidenceInterfaces';

const closeIconSrc = `${process.env.VITE_PROCESS_ENV_CDN_URL}/images/icons/close.svg`;
const shareToGoogleIconSrc = `${process.env.VITE_PROCESS_ENV_CDN_URL}/images/icons/icons-google-classroom-color.svg`;
...
export const ShareActivityPackModal = ({ activityPackData, closeModal, selectableClassrooms, selectedClassroomId, selectedClassroomName, singleActivity, unitId }: ShareActivityPackModalPropsInterface) => {
  ...
  function getDefaultLink() {
    ...
    if(singleActivity) {
      ...
      return `${process.env.VITE_DEFAULT_URL}/classroom_units/${id}/activities/${activityId}`;
    }
    if(!singleActivity) {
      const { id } = classroom;
      return `${process.env.VITE_DEFAULT_URL}/classrooms/${id}?unit_id=${unitId}`
    }
  }
  ...
  function handleClassChange(classOption: DropdownObjectInterface) {
    ...
    if(singleActivity) {
      ...
      const activityId = singleActivity.activityId || singleActivity.id
      setLink(`${process.env.VITE_DEFAULT_URL}/classroom_units/${classroomUnit.id}/activities/${activityId}`);
    } else {
      setLink(`${process.env.VITE_DEFAULT_URL}/classrooms/${value}?unit_id=${unitId}`);
    }
  }
