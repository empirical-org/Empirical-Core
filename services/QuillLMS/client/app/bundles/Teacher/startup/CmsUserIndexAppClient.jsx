...
export default class CmsUserIndex extends React.Component {
    ...
    return [
      {
        ...
        Cell: ({row}) => {
          if (row.original.school) {
            return <a href={`${process.env.VITE_DEFAULT_URL}/cms/schools/${row.original.school_id}`}>{row.original.school}</a>
          }
          return 'N/A';
        }
      }, {
        ...
        maxWidth: 40,
        Cell: ({row}) => {
          return <a href={`${process.env.VITE_DEFAULT_URL}/cms/users/${row.original.id}/edit`}>Edit</a>
        }
      }, {
        Header: "Details",
        accessor: 'details',
        ...
        maxWidth: 60,
        Cell: ({row}) => {
          return <a href={`${process.env.VITE_DEFAULT_URL}/cms/users/${row.original.id}`}>Details</a>
        }
      }, {
        Header: "Sign In",
        accessor: 'sign_in',
        ...
        maxWidth: 60,
        Cell: ({row}) => {
          return <a href={`${process.env.VITE_DEFAULT_URL}/cms/users/${row.original.id}/sign_in`}>Sign In</a>
        }
      }
    ];
  }
  ...
  search = (e) => {
    ...
    const link = `${process.env.VITE_DEFAULT_URL}/cms/users/search`
