...
const QuestionRow = (props: any) => (
  <tr>
    ...
    <td style={{width: "600px"}}><Link to={`/admin/${props.question.pathName}/${props.question.uid}/responses`}>{props.question.prompt}</Link></td>
    ...
    <td>{props.question.activities.length > 0 ? props.question.activities.map((a: any) => <Link key={props.question.uid} style={{ display: 'block' }} to={`/admin/lessons/${a.uid}`}>{a.name}</Link>) : ''}</td>
  </tr>
)
