...
export function startLesson(classroomUnitId: ClassroomUnitId, classroomSessionId: ClassroomSessionId, callback?: Function) {
  let url = new URL('/api/v1/classroom_units/classroom_teacher_and_coteacher_ids', process.env.VITE_DEFAULT_URL);
  const params = {
...
export const fetchActiveActivitySession = ({ sessionID, callback, }) => {
  const activeActivitySessionUrl = `${process.env.VITE_DEFAULT_URL}/api/v1/active_activity_sessions/${sessionID}`
...
export function finishActivity(
  ...
  fetch(`${process.env.VITE_DEFAULT_URL}/api/v1/classroom_units/finish_lesson`, {
    method: 'PUT',
    body: data,
    mode: 'cors',
    credentials: 'include',
...
export function getLessonData(
  ...
  return function(dispatch) {
    dispatch(getClassroomAndTeacherNameFromServer(classroomUnitId, classroomSessionId, process.env.VITE_DEFAULT_URL))
    dispatch(loadStudentNames(activityId, classroomUnitId, classroomSessionId, process.env.VITE_DEFAULT_URL))
    dispatch(loadFollowUpNameAndSupportingInfo(activityId, process.env.VITE_DEFAULT_URL, classroomSessionId))
  }
}
...
export function getPreviewData(
  ...
  const baseUrl:string = process.env.VITE_DEFAULT_URL ? String(process.env.VITE_DEFAULT_URL) : 'https://quill.org/'
  return function(dispatch) {
...
export function unpinActivityOnSaveAndExit(
  ...
  let url = new URL('/api/v1/classroom_units/unpin_and_lock_activity', process.env.VITE_DEFAULT_URL);
  const params = {
    activity_id: activityId,
    classroom_unit_id: classroomUnitId
  };
...
export function getClassroomAndTeacherNameFromServer(
  ...
  return function (dispatch) {
    let url = new URL('/api/v1/classroom_units/teacher_and_classroom_name', process.env.VITE_DEFAULT_URL);
    const params = {
      classroom_unit_id: classroomUnitId
    };
    Object.keys(params).forEach(key => url.searchParams.append(key, params[key]))
...
export function loadFollowUpNameAndSupportingInfo(
  ...
  return function (dispatch) {
    const coreUrl = baseUrl ? baseUrl : process.env.VITE_DEFAULT_URL
