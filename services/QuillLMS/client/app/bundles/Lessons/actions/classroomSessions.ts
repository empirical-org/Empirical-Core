...
export function startLesson(classroomUnitId: ClassroomUnitId, classroomSessionId: ClassroomSessionId, callback?: Function) {
  let url = new URL('/api/v1/classroom_units/classroom_teacher_and_coteacher_ids', import.meta.env.VITE_DEFAULT_URL);
  const params = {
...
export const fetchActiveActivitySession = ({ sessionID, callback, }) => {
  const activeActivitySessionUrl = `${import.meta.env.VITE_DEFAULT_URL}/api/v1/active_activity_sessions/${sessionID}`
...
export function finishActivity(
  ...
  fetch(`${import.meta.env.VITE_DEFAULT_URL}/api/v1/classroom_units/finish_lesson`, {
    method: 'PUT',
    body: data,
    mode: 'cors',
    credentials: 'include',
...
export function getLessonData(
  ...
  return function(dispatch) {
    dispatch(getClassroomAndTeacherNameFromServer(classroomUnitId, classroomSessionId, import.meta.env.VITE_DEFAULT_URL))
    dispatch(loadStudentNames(activityId, classroomUnitId, classroomSessionId, import.meta.env.VITE_DEFAULT_URL))
    dispatch(loadFollowUpNameAndSupportingInfo(activityId, import.meta.env.VITE_DEFAULT_URL, classroomSessionId))
  }
}
...
export function getPreviewData(
  ...
  const baseUrl:string = import.meta.env.VITE_DEFAULT_URL ? String(import.meta.env.VITE_DEFAULT_URL) : 'https://quill.org/'
  return function(dispatch) {
...
export function unpinActivityOnSaveAndExit(
  ...
  let url = new URL('/api/v1/classroom_units/unpin_and_lock_activity', import.meta.env.VITE_DEFAULT_URL);
  const params = {
    activity_id: activityId,
    classroom_unit_id: classroomUnitId
  };
...
export function getClassroomAndTeacherNameFromServer(
  ...
  return function (dispatch) {
    let url = new URL('/api/v1/classroom_units/teacher_and_classroom_name', import.meta.env.VITE_DEFAULT_URL);
    const params = {
      classroom_unit_id: classroomUnitId
    };
    Object.keys(params).forEach(key => url.searchParams.append(key, params[key]))
...
export function loadFollowUpNameAndSupportingInfo(
  ...
  return function (dispatch) {
    const coreUrl = baseUrl ? baseUrl : import.meta.env.VITE_DEFAULT_URL
