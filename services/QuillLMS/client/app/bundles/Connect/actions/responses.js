...
export function submitResponse(content, prid, isFirstAttempt) {
  ...
  return (dispatch) => {
    requestPost(
      `${process.env.VITE_CMS_URL}/responses/create_or_increment`,
...
export function uploadOptimalResponse(content, prid, isFirstAttempt) {
  ...
  return (dispatch) => {
    requestPost(
      `${process.env.VITE_CMS_URL}/responses/create_or_update`,
      { response: rubyConvertedResponse, },
      (body) => {
        dispatch({ type: C.DISPLAY_MESSAGE, message: 'Submission successfully saved!', });
      },
...
export function submitMassEditFeedback(ids, properties, qid) {
  return (dispatch) => {
    ...
    requestPut(
      `${process.env.VITE_CMS_URL}/responses/mass_edit/edit_many`,
...
export function massEditDeleteResponses(ids, qid) {
  return (dispatch) => {
    requestPost(
      `${process.env.VITE_CMS_URL}/responses/mass_edit/delete_many`,
      { ids, },
      (body) => {
        dispatch({ type: C.DISPLAY_MESSAGE, message: 'Submission successfully saved!', });
        dispatch({ type: C.SHOULD_RELOAD_RESPONSES, qid, });
...
export function getGradedResponsesWithCallback(questionID, callback) {
  requestGet(
    `${process.env.VITE_CMS_URL}/questions/${questionID}/responses`,
    (body) => {
      const bodyToObj = {};
      body.forEach((resp) => {
...
export function getMultipleChoiceResponseOptionsWithCallback(questionID, callback) {
  requestGet(`${process.env.VITE_CMS_URL}/questions/${questionID}/multiple_choice_options`,
    (body) => {
      const bodyToObj = {};
      body.forEach((resp) => {
        bodyToObj[resp.id] = resp;
