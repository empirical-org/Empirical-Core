...
export const getDistrictStandardsReports = (isFreemiumView) => {
  ...
  return (dispatch) => {
    requestGet(
      `${process.env.VITE_DEFAULT_URL}/api/v1/progress_reports/district_standards_reports${freemiumParam}`,
      (body) => {
        if (body.id && (process.env.VITE_PROCESS_ENV_RAILS_ENV === 'production')) {
