    ...
    RawSqlRunner.execute(
      <<-SQL
        SELECT
          ...
          CASE WHEN unit_owner.time_zone IS NOT NULL THEN unit_activities.publish_date at time zone unit_owner.time_zone >= NOW() at time zone unit_owner.time_zone ELSE unit_activities.publish_date >= NOW() END AS scheduled,
          MIN(acts.started_at) #{user_timezone_offset} AS started_at,
