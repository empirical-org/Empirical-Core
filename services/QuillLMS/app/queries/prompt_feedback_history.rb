class PromptFeedbackHistory
    ...
    def self.prompt_health_query(activity_id)
        FeedbackHistory.select(<<~SELECT
          prompt_id,
          COUNT(feedback_histories.id) AS total_responses,
          ...
          COUNT(DISTINCT CASE WHEN flag = 'repeated-consecutive' THEN feedback_histories.feedback_session_uid END) AS num_sessions_with_consecutive_repeated_rules,
          COUNT(DISTINCT CASE WHEN flag = 'repeated-non-consecutive' THEN feedback_histories.feedback_session_uid END) AS num_sessions_with_non_consecutive_repeated_rules,
    ...
    def self.serialize_results(results)
      serialized_rows = results.map do |result|
        result.serializable_hash(
          only: [:prompt_id, :total_responses, :session_count, :display_name, :num_final_attempt_optimal, :num_final_attempt_not_optimal, :avg_attempts_to_optimal, :num_sessions_with_consecutive_repeated_rule, :num_sessions_with_non_consecutive_repeated_rule, :num_first_attempt_optimal, :num_first_attempt_not_optimal],
          include: []
        )
      end
      serialized_rows.group_by { |r| r[:prompt_id]}
    end
end
