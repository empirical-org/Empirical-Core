class ProgressReports::StudentOverview
  def self.results(classroom_id, student_id)
    ActiveRecord::Base.connection.execute(
      "SELECT EXTRACT(EPOCH FROM activity_sessions.completed_at) AS completed_at, activity_sessions.id AS activity_sessions_id, activities.name, activity_sessions.percentage, units.name AS unit_name, activities.activity_classification_id, classroom_units.completed AS is_a_completed_lesson, activities.id AS activity_id, classroom_units.id AS classroom_unit_id
      FROM classroom_units
       ...
       GROUP BY units.name, activity_sessions.completed_at, activities.name, activity_sessions.percentage, classroom_units.completed, activity_sessions.id, activities.activity_classification_id, activities.id, units.id, classroom_units.id
