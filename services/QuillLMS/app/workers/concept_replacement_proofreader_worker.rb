class ConceptReplacementProofreaderWorker
  ...
  def perform(original_concept_uid, new_concept_uid)
    activities = HTTParty.get("#{ENV['FIREBASE_DATABASE_URL']}/v3/passageProofreadings").parsed_response
    activities.each do |key, act|
      if act['passage'].include?(original_concept_uid)
        ...
        HTTParty.put("#{ENV['FIREBASE_DATABASE_URL']}/v3/passageProofreadings/#{key}", body: new_passage_hash.to_json)
      end
    end
  end
