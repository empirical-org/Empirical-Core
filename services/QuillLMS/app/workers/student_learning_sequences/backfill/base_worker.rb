...
module StudentLearningSequences
  module Backfill
    class BaseWorker
      ...
      def classroom_units = ClassroomUnit.joins(unit: :activities).select(:id, :assigned_student_ids).order(:id)
      ...
      def backfill_classroom_units(query)
        fetch_page(query).find_each do |classroom_unit|
          ...
          classroom_unit.assigned_student_ids&.uniq&.each do |student_id|
