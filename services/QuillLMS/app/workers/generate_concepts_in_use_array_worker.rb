class GenerateConceptsInUseArrayWorker
  ...
  def get_concepts_in_use
    concepts = ActiveRecord::Base.connection.execute("
      ...
      LEFT JOIN activity_sessions ON concept_results.activity_session_id = activity_sessions.id AND activity_sessions.completed_at > (CURRENT_DATE - INTERVAL '3 months')
