  ...
  private def diagnostics_organized_by_classroom
    ...
    diagnostic_records.each do |record|
      ...
      if record['post_test_id']
        ...
        grouped_record[:pre] = post_test || { activity_name: Activity.find_by_id(record['post_test_id'])&.name, unit_template_id: ActivitiesUnitTemplate.find_by_activity_id(record['post_test_id'])&.unit_template_id }
      else
