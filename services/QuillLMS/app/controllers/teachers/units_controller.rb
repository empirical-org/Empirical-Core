    ...
    units = RawSqlRunner.execute(
      <<-SQL
        SELECT
          ...
          ua.publish_date >= NOW() at time zone 'utc' AS scheduled,
          NOW() as now_with_no_time_zone,
          NOW() at time zone 'utc' AS now_in_utc,
          NOW() at time zone unit_owner.time_zone AS now_in_est,
          ua.publish_date AS publish_date_with_no_time_zone,
          ua.publish_date at time zone 'utc' AS publish_date_in_utc,
          ua.publish_date at time zone unit_owner.time_zone AS publish_date_in_est,
          state.completed,
