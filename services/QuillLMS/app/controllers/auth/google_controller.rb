class Auth::GoogleController < ApplicationController
  ...
  def set_user
    if route_redirects_to_my_account?(session[GOOGLE_REDIRECT]) || route_redirects_to_classrooms_index?(session[GOOGLE_REDIRECT]) || route_redirects_to_assign_activities?(session[GOOGLE_REDIRECT])
