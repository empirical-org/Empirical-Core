class RuleFeedbackHistory
    ...
    def self.exec_query(conjunction:, activity_id:)
        sql = <<~SQL
            select rules.uid, MAX(prompts.activity_id) as activity_id, MAX(rules.rule_type) as rule_type, MAX(rules.suborder) as rule_suborder, MAX(rules.name) as rule_name, count(feedbacks.id) as feedback_histories_count
