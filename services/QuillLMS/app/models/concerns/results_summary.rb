module ResultsSummary
  ...
  def results_summary(activity_id, classroom_id, unit_id)
    ...
    set_activity_sessions_and_assigned_students
  ...
  private def set_activity_sessions_and_assigned_students
    if unit_id
      ...
      unit_ids = current_user.units.joins("unit_activities ON unit_activities.activity_id = ?", activity_id)
  ...
  private def student_results
    @assigned_students.map do |assigned_student|
      activity_session = @activity_sessions.find_by(user_id: assigned_student.id)
      if activity_session
        {
