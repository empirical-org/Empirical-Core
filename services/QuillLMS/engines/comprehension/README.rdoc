...
A Rails Engine to provide the API endpoints for the Quill Comprehension app. This will live inside the QuillLMS Rails application. Engines are a way to compartmentalize code without having to host a separate app.
...
Configure the parent_activity in parent rails app, in a `config/initializer/comprehension.rb` file:
...
`rake comprehension:install:migrations`
