...
module Comprehension
  RSpec.describe(RegexCheck, :type => :model) do
    ...
    context("#feedback_object") do
      it("return optimal blank feedback when there is no regex match") do
        $redis.redis.flushdb
      ...
      it("be false when there is a regex match") do
      ...
      it("return the highest priority feedback when two feedbacks exist") do
