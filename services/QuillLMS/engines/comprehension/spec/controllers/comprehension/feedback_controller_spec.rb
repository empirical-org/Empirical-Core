...
module Comprehension
  RSpec.describe(FeedbackController, :type => :controller) do
    ...
    context("plagiarism") do
      ...
      it("return 404 if prompt id does not exist") do
      ...
      it("return successfully when there is plagiarism") do
    ...
    context("regex") do
      ...
      it("return 404 if rule_type does not exist") do
      ...
      it("return successfully when there is regex feedback") do
    ...
    context("#automl") do
      ...
      it("return 404 if prompt has no associated automl_model") do
        post("automl", :params => ({ :entry => "some text", :prompt_id => @prompt.id, :session_id => 1, :previous_feedback => ([]) }), :as => :json)
        expect(404).to(eq(response.status))
      end
      it("return feedback payloads based on the lib matched_rule value") do
        entry = "entry"
    ...
    context("#spelling") do
      it("return correct spelling feedback when endpoint returns 200") do
      ...
      it("return 500 if there is an error on the bing API") do
