...
module Comprehension
  RSpec.describe(TurkingRound, :type => :model) do
    context("validations") do
      ...
      it("validate presence of uuid on any update call") do
      ...
      it("set a default uuid when creating a new record") do
        turking_round = create(:comprehension_turking_round, :uuid => nil)
        expect(turking_round.uuid).to_not(be_nil)
      end
      it("ensure expires_at is a future date on create") do
      ...
      it("allow updates to expires_at to be in the past even though creates should not") do
        turking_round = create(:comprehension_turking_round)
