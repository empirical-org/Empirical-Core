...
module Evidence
  module GenAI
    class PromptBuilder < ApplicationService
      ...
      attr_reader :prompt, :history, :template_file, :entry, :options

      def initialize(prompt:, history: [], template_file: nil, entry: nil, options: {})
        @prompt = prompt
        @history = history
      ...
      private def highlight_texts
        prompt
          ...
          .map.with_index { |text_array, i| "#{i + 1}. #{strip_tags(text_array.join(' '))}" }
          .join("\n")
      end
    end
  end
