...
module Evidence
  class GenAiRecordBuilder < ApplicationService
    ...
    def create_or_update_gen_ai_activity(prompt, relevant_text_key)
      gen_ai_activity = prompt.stem_vaults&.first&.activity || Evidence::Research::GenAI::Activity.find_or_initialize_by(name: activity.title)
    ...
    def create_or_update_stem_vault(prompt, gen_ai_activity)
      ...
      partitioned_stem = prompt.text.rpartition(prompt.conjunction).first
      stem_vault.stem = partitioned_stem.present? ? partitioned_stem.strip : prompt.text.strip
