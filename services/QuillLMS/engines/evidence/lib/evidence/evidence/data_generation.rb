...
For players like Jones, instant replays can lead to heartbreak—but at the end of the day, they’re just part of the modern game. “I felt like I was in. I felt like I scored,” he told reporters after the game. \“But it’s not my place to do [the replay officials’] job. They’re professionals at what they do.\""
    ...
    def run
      ...
      run_prompt(prompt: prompt, count: MAIN_COUNT, seed: 'full_passage')
      # whole passage plus prompt for each noun
      nouns.each do |noun|
        prompt = prompt_text(context: passage, noun: noun)
        run_prompt(prompt: prompt, count: NOUN_COUNT, noun: noun, seed: "full_passage_noun_#{noun.gsub(SPACE,BLANK)}")
      end
      ...
      split_passage.each.with_index do |text_chunk, index|
        ...
        run_prompt(prompt: prompt, count: MAIN_COUNT, seed: "text_chunk_#{index + 1}")

        nouns.each do |noun|
          prompt = prompt_text(context: text_chunk, noun: noun)
          run_prompt(prompt: prompt, count: NOUN_COUNT, noun: noun, seed: "text_chunk_#{index + 1}_noun_#{noun.gsub(SPACE,BLANK)}")
        end
      end
    ...
    def run_prompt(prompt:, count:, seed:, noun: nil)
      output = Evidence::OpenAI.new(prompt: prompt, count: count).request
      new_results = output.map {|s| Result.new(text: noun.nil? ? s : [noun, s].join(SPACE), seed: seed)}
