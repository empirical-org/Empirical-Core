  ...
  RSpec.describe(AutomlModelsController, :type => :controller) do
    ...
    describe '#index' do
      ...
      context 'should with automl_models' do
        ...
        it 'should return successfully' do
          ...
          expect(parsed_response.first["external_id"]).to(eq(automl_model.external_id))
    ...
    describe '#create' do
      ...
      it 'should create a valid record and return it as json' do
        AutomlModel.stub_any_instance(:automl_name, automl_model.name) do
          AutomlModel.stub_any_instance(:automl_labels, automl_model.labels) do
            post(:create, :params => ({ :automl_model => ({ :prompt_id => automl_model.prompt_id, :external_id => automl_model.external_id }) }))
          end
        end

        expect(response.code.to_i).to(eq(201))
        expect(parsed_response["external_id"]).to(eq(automl_model.external_id))
      ...
      it 'should create new records with state = inactive no matter what is passed in' do
        AutomlModel.stub_any_instance(:automl_name, automl_model.name) do
          AutomlModel.stub_any_instance(:automl_labels, automl_model.labels) do
            post(:create, :params => ({ :automl_model => ({ :prompt_id => automl_model.prompt_id, :external_id => automl_model.external_id, :state => (AutomlModel::STATE_ACTIVE) }) }))
          end
        end
      ...
      it 'should not create an invalid record and return errors as json' do
        AutomlModel.stub_any_instance(:automl_name, automl_model.name) do
          AutomlModel.stub_any_instance(:automl_labels, automl_model.labels) do
            post(:create, :params => ({ :automl_model => ({ :prompt_id => automl_model.prompt_id, :external_id => "" }) }))
          end
        end
        parsed_response = JSON.parse(response.body)
        expect(response.code.to_i).to(eq(422))
        expect(parsed_response["external_id"].include?("can't be blank")).to(eq(true))
    ...
    describe '#update' do
      ...
      it 'should not update read-only attributes return 200' do
        old_id = automl_model.external_id
        patch(:update, :params => ({ :id => automl_model.id, :automl_model => ({ :external_id => "anything", :name => "anything", :labels => (["anything"]) }) }))
        expect(response.code.to_i).to(eq(200))
        expect(automl_model.id).to(eq(JSON.parse(response.body)["id"]))
        automl_model.reload
        expect(old_id).to(eq(automl_model.external_id))
      end
    end
