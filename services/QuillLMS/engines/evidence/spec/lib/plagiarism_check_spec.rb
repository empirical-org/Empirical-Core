    ...
    context 'benchmarking', :benchmarking do
      ...
      it '#known slow API calls' do
        ...
        samples = [
          ...
          ["Some Native Hawaiians are moving to Las Vegas because Hawaii has a higher cost of living in the nation, examples food, housing, clothing, electricity and other individuals cost more in Hawaii than in the United States.", plagiarism2, true] # 2620ms in prod
        ]
        runtime = Benchmark.realtime do
          samples.each do |entry, plagiarism_text, optimal|
            expect(Evidence::PlagiarismCheck.new(entry, plagiarism_text, '', rule).feedback_object[:optimal]).to eq(optimal)
          end
        end
        puts format('Average known slow API runtime: %<runtime>.3f seconds', {runtime: (runtime / samples.length)})
      end
    end
  end
end
