...
module Evidence
  RSpec.describe(FuzzyPlagiarismCheck, :type => :model) do
    ...
    context 'should #feedback_object' do
      ...
      it 'should return appropriate feedback attributes if there is plagiarism' do
        $redis.redis.flushdb
        entry = "these are some repsonse TYPOwords to plagiarize and this is plagiarism"
        passage = "these are some repsonse words to plagiarize and this is plagiarism"
        feedback = "this is some standard plagiarism feedback"
        plagiarism_check = Evidence::FuzzyPlagiarismCheck.new(entry, passage, feedback, rule)
        ...
        expect(feedback[:fuzzy_plagiarism_match]).to be(true)
      end
      ...
      context 'space normalization handling' do
        ...
        it 'should successfully highlight even when the user entry has multiple consecutive spaces but the passage does not' do
          ...
          expect(feedback[:highlight][0][:text]).to eq("from  the    passage even     though it  has a ton of TYPOspaces")
