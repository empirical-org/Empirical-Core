...
RSpec.shared_context 'Pre Post Diagnostic Skill Group Performance View' do
  ...
  let(:pre_diagnostic_units) { create_list(:unit, classroom_count, activities: [pre_diagnostic]) }
  let(:post_diagnostic_units) { create_list(:unit, classroom_count, activities: [post_diagnostic]) }

  let(:pre_diagnostic_classroom_units) { pre_diagnostic_units.map.with_index { |unit, i| create(:classroom_unit, classroom: classrooms[i], unit: unit, assigned_student_ids: pre_diagnostic_assigned_students.map(&:id)) } }
