...
describe AdminAccessController do
  ...
  describe '#request_upgrade_to_admin_from_existing_admins' do
    it 'creates an admin info record with the requisite information, an admin approval request record, and fires the necessary segment workers' do
      ...
      expect(AdminReceivedAdminUpgradeRequestFromTeacherAnalyticsWorker).to receive(:perform_async).with(school_admin.user.id.to_s, user.id, reason)
      expect(TeacherRequestedToBecomeAdminAnalyticsWorker).to receive(:perform_async).with(user.id)

      post :request_upgrade_to_admin_from_existing_admins, params: { admin_ids: [school_admin.user.id], reason: reason }
