...
describe Api::V2::TitleCardsController, type: :controller do
  ...
  describe "#update" do
    ...
    it 'should update the specified TitleCard if all data is valid and return it' do
      ...
      put :update, uid: title_card.uid, title_card: {content: new_content}
      expect(response.status).to eq(200)
    ...
    it 'should 422 UNPROCESSABLE ENTITY if the posted data is invalid' do
      invalid_content = nil
      put :update, uid: title_card.uid, title_card: {content: invalid_content}
    ...
    it 'should 409 CONFLICT if the provided UID already belongs to an existing TitleCard' do
      ...
      put :update, uid: title_card.uid, title_card: {uid: used_uid}
  ...
  describe "#destroy" do
    ...
    it 'should delete TitleCards if found' do
      delete :destroy, uid: title_card.uid
      expect(response.status).to eq(200)
    ...
    it 'should 404 NOT FOUND if no TitleCard with the provided UID is found' do
      delete :destroy, uid: 'doesnotexist'
      expect(response.status).to eq(404)
      expect(response.body).to eq('')
    end
