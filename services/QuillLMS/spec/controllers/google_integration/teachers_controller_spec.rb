...
module GoogleIntegration
  RSpec.describe TeachersController do
    ...
    context '#import_classrooms' do
      ...
      it do
        expect(ImportTeacherClassroomsStudentsWorker).to receive(:perform_async)
        expect(TeacherClassroomsCache).to receive(:delete).with(teacher.id)
        expect(HydrateTeacherClassroomsCacheWorker).to receive(:perform_async).with(teacher.id)
        subject
      end
    ...
    describe '#import_students'  do
      ...
      context 'import google classroom student flow' do
        ...
        it 'should kick off background job that imports students' do
          ...
          expect(ImportTeacherClassroomsStudentsWorker)
    ...
    describe '#retreive_clasrooms' do
      ...
      context 'user is google authorized' do
        ...
        context 'teacher classrooms cache has data' do
          ...
          before { allow(TeacherClassroomsCache).to receive(:read).and_return(data.to_json) }
