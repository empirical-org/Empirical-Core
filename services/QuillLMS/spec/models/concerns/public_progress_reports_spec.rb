...
describe PublicProgressReports, type: :model do
  ...
  describe '#results_for_classroom' do
    ...
    it 'populates directions and last feedback' do
      activity_session_2 = create(:activity_session_without_concept_results, classroom_unit_id: classroom_unit.id, activity_id: activity.id, user: students_classrooms1.student)

      lastFeedback = "This is the last feedback the student received."
      directions = "Combine the sentences."
      create(:concept_result, activity_session: activity_session_2, metadata: { "attemptNumber": 1, "answer": 'Arbitrary sample incorrect answer.', "correct": 0, "directions": directions, "prompt": "prompt", "questionNumber": 1, "questionScore": 0.75})
      create(:concept_result, activity_session: activity_session_2, metadata: { "attemptNumber": 2, "answer": 'Arbitrary sample correct answer.', "correct": 1, "lastFeedback": lastFeedback, "directions": directions, "prompt": "prompt", "questionNumber": 1, "questionScore": 0.75})
      report = FakeReports.new.results_for_classroom(classroom_unit.unit_id, activity.id, classroom.id)
      ...
      expect(concept_results[1][:lastFeedback]).to eq(lastFeedback)
    end
