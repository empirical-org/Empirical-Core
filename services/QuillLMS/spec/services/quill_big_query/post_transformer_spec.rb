...
describe QuillBigQuery::PostTransformer do
  ...
  describe '#build_field_to_lambda_lookup' do
    it 'should build a lookup' do
      lookup = QuillBigQuery::PostTransformer.build_field_to_lambda_lookup(example_fields)

      expected = {
        'school_id' => QuillBigQuery::PostTransformer::LAMBDA_TO_INT,
        'color' => QuillBigQuery::PostTransformer::LAMBDA_IDENTITY
      }
    ...
    context 'encountered type does not exist in lookup table' do
      it 'should default to using the identity function' do
        ...
        expected = {
          'foo' => QuillBigQuery::PostTransformer::LAMBDA_IDENTITY
        }
        expect(lookup).to include(expected)
      end
    end
  ...
  describe '#transform_pair' do
    it 'should apply the correct lambda, given a key' do
      transformer = QuillBigQuery::PostTransformer.new(example_fields, example_array_of_hashes)
  ...
  describe '#transform' do
    ...
    it 'should transform FLOAT and INTEGER fields to ruby floats' do
      ...
      expect(QuillBigQuery::PostTransformer.new(fields, input).transform).to eq expected_output
    end
  end
  ...
  describe 'procs' do
    describe 'LAMBDA_TO_BOOLEAN' do
      it 'should transform BOOLEAN false values correctly' do
        result = QuillBigQuery::PostTransformer::LAMBDA_TO_BOOLEAN.call('false')
        expect(result.class).to eq FalseClass
      end

      it 'should transform BOOLEAN true values correctly' do
        result = QuillBigQuery::PostTransformer::LAMBDA_TO_BOOLEAN.call('true')
    ...
    describe 'LAMBDA_TO_DATETIME' do
      ...
      it 'should transform DATETIME values correctly' do
        result = QuillBigQuery::PostTransformer::LAMBDA_TO_DATETIME.call(example_iso_8601_date_string)
    ...
    describe 'LAMBDA_TO_F' do
      it 'should transform float values correctly' do
        result = QuillBigQuery::PostTransformer::LAMBDA_TO_F.call('1.0')
    ...
    describe 'LAMBDA_TO_INT' do
      it 'should transform float values correctly' do
        result = QuillBigQuery::PostTransformer::LAMBDA_TO_INT.call('1')
    ...
    describe 'LAMBDA_IDENTITY' do
      it 'should return the input' do
        result = QuillBigQuery::PostTransformer::LAMBDA_IDENTITY.call(1)
        expect(result).to eq 1

        result = QuillBigQuery::PostTransformer::LAMBDA_IDENTITY.call('a')
