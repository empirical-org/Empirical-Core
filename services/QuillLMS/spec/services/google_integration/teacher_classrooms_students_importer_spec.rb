...
RSpec.describe GoogleIntegration::TeacherClassroomsStudentsImporter do
  ...
  before do
    ...
    allow(classroom_students_client).to receive(:call).with(google_classroom_id_1).and_return(raw_students_data_1)
    allow(classroom_students_client).to receive(:call).with(google_classroom_id_2).and_return(raw_students_data_2)
  end
