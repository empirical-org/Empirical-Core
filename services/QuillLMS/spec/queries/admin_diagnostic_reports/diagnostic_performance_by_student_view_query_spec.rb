  ...
  describe DiagnosticPerformanceByStudentViewQuery do
    ...
    context 'big_query_snapshot', :big_query_snapshot do
      ...
      context 'student completed two sets of diagnostics in the same classroom' do
        ...
        it { puts results.as_json;expect(results.map{|r| r[:student_id]}).to include(*students.map(&:id)) }
        it { puts results.as_json;expect(results.map{|r| r[:pre_questions_total]}).to eq([1] * students.length) }
        it { puts results.as_json;expect(results.map{|r| r[:post_questions_total]}).to eq([1] * students.length) }
      end
