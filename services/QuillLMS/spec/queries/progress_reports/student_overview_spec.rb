...
describe 'StudentOverview' do
  ...
  it "returns at least as many rows as the student was assigned activities within that classroom" do
    assigned_activity_count = ActiveRecord::Base.connection.execute("SELECT COUNT(classroom_activities.id) FROM classroom_activities WHERE #{student.id} = ANY (classroom_activities.assigned_student_ids::int[])").to_a.first['count'].to_i
