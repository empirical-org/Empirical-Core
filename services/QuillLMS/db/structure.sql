...
CREATE SEQUENCE partner_contents_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
ALTER SEQUENCE partner_contents_id_seq OWNED BY partner_contents.id;
...
CREATE SEQUENCE third_party_user_ids_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
ALTER SEQUENCE third_party_user_ids_id_seq OWNED BY third_party_user_ids.id;
...
ALTER TABLE ONLY partner_contents ALTER COLUMN id SET DEFAULT nextval('partner_contents_id_seq'::regclass);
...
ALTER TABLE ONLY third_party_user_ids ALTER COLUMN id SET DEFAULT nextval('third_party_user_ids_id_seq'::regclass);
...
ALTER TABLE ONLY partner_contents
...
ALTER TABLE ONLY third_party_user_ids
...
CREATE INDEX index_activity_session_interaction_logs_on_activity_session_id ON activity_session_interaction_logs USING btree (activity_session_id);
...
CREATE INDEX index_activity_sessions_on_classroom_unit_id ON activity_sessions USING btree (classroom_unit_id);
...
CREATE INDEX index_change_logs_on_changed_record_type_and_changed_record_id ON change_logs USING btree (changed_record_type, changed_record_id);
...
CREATE INDEX index_change_logs_on_user_id ON change_logs USING btree (user_id);
...
CREATE INDEX index_concept_results_on_concept_id ON concept_results USING btree (concept_id);
...
CREATE INDEX index_partner_contents_on_content_type_and_content_id ON partner_contents USING btree (content_type, content_id);
...
CREATE INDEX index_partner_contents_on_partner ON partner_contents USING btree (partner);
...
CREATE INDEX index_schools_on_mail_zipcode ON schools USING btree (mail_zipcode);
...
CREATE INDEX index_users_on_clever_id ON users USING btree (clever_id);
...
CREATE UNIQUE INDEX index_zipcode_infos_on_zipcode ON zipcode_infos USING btree (zipcode);
...
ALTER TABLE ONLY change_logs
    ADD CONSTRAINT fk_rails_1a847a1740 FOREIGN KEY (user_id) REFERENCES users(id);
...
ALTER TABLE ONLY third_party_user_ids
    ADD CONSTRAINT fk_rails_aca4adc66a FOREIGN KEY (user_id) REFERENCES users(id);
