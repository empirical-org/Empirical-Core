...
class ResponsesController < ApplicationController
  ...
  def multiple_choice_options
    multiple_choice_options = Rails.cache.fetch("questions/#{params[:question_uid]}/multiple_choice_options", :expires_in => 900) do
      optimal_responses = Response.where(question_uid: "-KQS6-2JxSzGYB2MERYc", optimal: true).order('count DESC').limit(2).to_a
      sub_optimal_responses = Response.where(question_uid: "-KQS6-2JxSzGYB2MERYc", optimal: false).order('count DESC').limit(2).to_a
