...
export class StudentViewContainer extends React.Component<StudentViewContainerProps, StudentViewContainerState> {
  ...
  formatHtmlForPassage = () => {
    ...
    let html = currentActivity.passages.map(passage => {
      const paragraphArray = passage.match(/[^\r\n]+/g)
      return paragraphArray.map(p => `<p>${p}</p>`).join('')
    })

    if (!activeStep || activeStep === READ_PASSAGE_STEP) { return html }
    ...
    if (!(submittedResponsesForActivePrompt && submittedResponsesForActivePrompt.length)) { return html }
    ...
    passageHighlights.forEach(hl => {
      html = html.map(passage => passage.replace(hl.text, `<span class="passage-highlight">${hl.text}</span>`))
    })
