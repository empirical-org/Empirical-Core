...
class PlayClassroomLessonContainer extends React.Component<any, any> {
  ...
  componentWillReceiveProps(nextProps, nextState) {
    ...
    const npCSData = nextProps.session
    ...
    } else if (this.props.session.id && student && this.studentEnrolledInClass(student)) {
      registerPresence(this.props.session.id, student);
    } else {
      if (!this.studentEnrolledInClass(student)) {
        if (nextProps.session.public) {
  ...
  handleKeyDown(event) {
    ...
    if (tag !== 'input' && tag !== 'textarea' && className.indexOf("drafteditor") === -1 && (event.keyCode === 39 || event.keyCode === 37)) {
      const classroomSessionId: ClassroomSessionId|null = this.props.session.id
  ...
  studentEnrolledInClass(student: string|null) {
    return student && this.props.session.students ? !!this.props.session.students[student] : false
  }

  handleStudentSubmission(data: string) {
    const classroomSessionId: ClassroomSessionId|null = this.props.session.id;
    const student: string|null = getParameterByName('student');
    const currentSlide: string = this.props.session.current_slide;
  ...
  renderLeftButton() {
    if (getParameterByName('projector') && this.props.session.current_slide !== '0') {
      const classroomSessionId: ClassroomSessionId|null = this.props.session.id;
      const sessionData: ClassroomLessonSession = this.props.session;
      const editionData: CustomizeIntf.EditionQuestions = this.props.customize.editionQuestions;
  ...
  renderRightButton() {
    const currentSlide = Number(this.props.session.current_slide)
  ...
  public render() {
    if (this.state.shouldEnterName) {
      return (
       ...
       const editionDataLoaded: boolean = true;
       // const data: ClassroomLessonSessions  = this.props.session;
       // const hasreceiveddata = this.props.classroomSessions.hasreceiveddata
       const absentTeacher = this.props.session.absentTeacherState ? <CLAbsentTeacher /> : null
       const watchTeacher = this.props.session.watchTeacherState && !this.state.projector ? <CLWatchTeacher /> : null

       if (this.props.session && lessonDataLoaded && editionDataLoaded) {
         const component = this.renderCurrentSlide(this.props.session, lessonData, editionData);
         if (component) {
           return (
             <div>
              <WakeLock />
